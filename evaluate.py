...
def evaluate_BC_with_fallback(ROWS, COLS, eval_mps, mode = BCMode.ASTAR_FALLBACK):
    ...
    for grid, start, goal in eval_mps:
        ...
        if bc_agent.pos == goal:
            success += 1
            tot_steps += steps
            fallback_steps += bc_runner.fallback_steps
        else:
            timeout += 1
        
    avrg_steps = tot_steps / success if success != 0 else None
    fallback_usage = (fallback_steps / len(eval_mps)) / avrg_steps if avrg_steps != 0 else None
    return success, timeout, avrg_steps, fallback_usage
...
if __name__ == '__main__':
    ...
    bc_fallback_success, bc_fallback_timeout, bc_fallback_avrg_steps, bc_fallback_usage = evaluate_BC_with_fallback(15, 15, eval_mps)
    print("Astar:")
    ...
    print("success rate:", bc_fallback_success / len(eval_mps), " timeout:", bc_fallback_timeout, " average steps:", bc_fallback_avrg_steps, "fallback usage:", bc_fallback_usage)
